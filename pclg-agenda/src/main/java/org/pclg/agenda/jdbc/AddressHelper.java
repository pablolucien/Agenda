...
final class AddressHelper implements FieldManagerHelper {
    private static final String INSERT_INTO_DIRECCION_SENTENCE =
        "INSERT INTO DIRECCION (Clave, Version,  Secuencia, Direccion) VALUES (?, ?, ?, ?)";

    private static final String SELECT_FROM_DIRECCION_SENTENCE =
        "SELECT Direccion FROM DIRECCION WHERE Clave = ? AND Version = ? " + GeneralHelper.ORDER_BY_SECUENCIA_CLAUSE;

    private final GeneralHelper generalHelper;
    private final PreparedStatement selectStatement;
    private final PreparedStatement insertStatement;

    AddressHelper(final Connection dbConnection, final GeneralHelper generalHelper) throws SQLException {
        this.generalHelper = generalHelper;
        selectStatement = dbConnection.prepareStatement(SELECT_FROM_DIRECCION_SENTENCE);
        insertStatement = dbConnection.prepareStatement(INSERT_INTO_DIRECCION_SENTENCE);
    }
    ...
    public void retrieve(final AgendaRecord record) throws SQLException {
        ...
        selectStatement.setInt(2, record.getVersionAddress());
    ...
    public int persist(final AgendaRecord record) throws SQLException {
        ...
        if (!isEmptyOrBlank(direccion)) {
            ...
            for (int ii = 1; ii <= nrChunks; ii++) {
                ...
                insertStatement.setInt(2, newVersionDireccion);
                insertStatement.setInt(3, ii);
                insertStatement.setString(4, direccion.substring((ii - 1) * AgendaRecord.ADDRESS_FIELD_LEN,
