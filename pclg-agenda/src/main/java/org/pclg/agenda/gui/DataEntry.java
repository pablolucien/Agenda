                ...
                if (dirTree.accepted()) {
                    dirTree.getFile().ifPresent(ff -> {
                        ...
                        try {
                            ...
                            agendaDb.updateImagePath(source.toString().replace("\\", "/").substring(imagesRoot.length()), newPath);
                            if (originalRecord != null && getData().getVersion() == originalRecord.getVersion()) {
