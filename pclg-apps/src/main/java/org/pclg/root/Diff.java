...
public final class Diff {
	...
	private static void checkDirs(final boolean combine, final File newDir,
			final File oldDir,
			final File diffDir) {
		// Sanity check. Se podr�a hacer con assert
	...
	/**
	 * Inicializa la medicion de tiempos, si est� disponible.
	 *
	 * @since 2003.05.10
	 */
	...
	/**
	 * Comienza la medicion de tiempos, si est� disponible.
	 *
	 * @since 2003.05.10
	 */
	...
	/**
	 * Informa sobre la medicion de tiempos, si est� disponible.
	 *
	 * @since 2003.05.10
	 */
		...
		try {
			...
			while (adding) {
				try {
					...
					adding = false;
					//ex.printStackTrace();
					System.out.println("Se acab� la entrada");
				}
			}
...
//Poner 0 en los espacios vacios de kazaa, para que comprima mejor
		...
		for (int i = 0; i < files.length; i++) {
			...
			try {
				...
				while ((count = fis.read(buffer)) > 0) {
					// Limpiar el fin del buffer si la lectura no lo llen�
	...
	private static void compareDigest(final String dirName1,
		...
		for (int i = 0; i < files1.length; i++) {
			...
			if (files1[i].length() != file2.length()) {
				System.out.println("Son de tama�os diferentes");
			}
	...
	/**
	 * Ayuda al usuario
	 * Al ser un m�todo publico, puede ser llamado por cualquier otra clase, en particular un sistema de ayuda
	 * <p/>
	 * --author El Coyote Cojo
	 * @since 2003.abr.25 12:02:27, CEST
	 */
