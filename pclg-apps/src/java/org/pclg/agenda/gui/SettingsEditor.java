...
/**
 * Para Encapsular la edición de los settings, y si me acuerdo en el futuro, reutilizarlo.
 *
 * @author El Coyote.
 * @since 5/02/16 0:52
 */
class SettingsEditor {
	...
	void editSettings() {
		...
		if (propertiesPanel.editMap(PropertiesHelper.getStringFromProperties(
			...
			for (final MapEditor.Property editableProperty : EDITABLE_PROPERTIES) {
				// Ponerla aquí para que sea tomada en cuenta ahora.
				properties.setProperty(editableProperty.getName(), newMap.get(editableProperty.getName()));
				// Ponerla aquí para que sea guardada para próximas ejecuciones.
