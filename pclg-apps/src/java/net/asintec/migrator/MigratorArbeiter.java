...
abstract class MigratorArbeiter extends Thread implements MigratorConstants {
	...
	*/
	...
	/** La conexion con la base de datos. No es usada aquí, pero seguramente sí por quien extienda esta clase */
	...
	/**
		Determina de un modo bastante naïf si un texto es una afirmacion
		@author El Coyote Cojo
	...
	*/
	...
	/**
		Hace toda la preparación necesaria para una migración (asignación de variables, construccion de las sentencias SQL, etc.)
		<br>????? este metodo y run deberían ser synchronized
	*/
...
//Ojo: revisar el comportamiento de esto cuando es una migracion en batch (aunque ahora los batch los debería hacer MigratorDaemon)
		if(migrating) {
...
//		debug(this.tiposOrg);
		...
		// hacerse a mano cuando se hace una duplicación
		...
		for(int i = 0; i < migrationInfo.getRowCount(); i++) {
			...
			if (srcField.equals(CONSTANTE)) {
				...
				// 2002.05.20 Access permite espacios el los nombres y tambien permite el guión
...
//ejemplo: C:\home\development\modules\src\java\org\pclg\migrator\MySQL-MySQL_Cifrar_contactos.conf
			...
			if (preprocessorName != null && !preprocessorName.equals("")) {
				try {
					final Preprocessor preprocessor = (Preprocessor) Class.forName(preprocessorName).newInstance();		// ??? aqui debería haber una factory
		...
		// Ahora que está todo armado, comenzamos la migración
		start();
	}
	...
	*/
	void fillStatement(final PreparedStatement pstmt, final int index,
		...
		if(data == null) {
			//logError("Valor nulo en la posición " + index + " (" + columna + ")");
