...
final class CompDelWorker extends Thread {
	...
	// 2002.02.26 Un Hack para ahorrar tiempo: no pedir el dir ni el digest si ya est� hecho
    ...
    /**
     * Indica que debemos abortar la operaci�n.
     *
     * @since 2006.07.29
     */
    void terminate() {
    ...
    /**
     * De momento est� como una chapucilla. (�Hasta cu�ndo seguir� as�? nice
     * question!).
     *
     * @since 2003.11.11 Cumplea�os de Mercily
     */
    ...
    private String formatExcludeList() {
        ...
        stringBuilder.append("Lista de exclusi�n:");
    ...
    /**
     * Does the work.
     * --param theSlave De momento est� como una chapucilla. (�Hasta cu�ndo
     *                 seguir� as�? nice question!) desde 2003.11.11 Cumplea�os
     *                 de Mercily.
    ...
    private void doTheWork() throws FileNotFoundException {
        ...
        if (!compDel.frame.optionPanel.fastHeuristicsOnly.isSelected()) {
            ...
            for (ii = 0; ii < digest1.length && !mustDie; ii++) {
                ...
                for (jj = 0; jj < digest2.length; jj++) {
                    beNice();
                    if (digest2[jj] == null
                        || archivo1.get(ii).equals(archivo2.get(jj))  // !!! Importante: no borrarlo si estoy comparando contra �l mismo
                    ...
                    if (FileTools.compareContents(archivo1.get(ii), archivo2.get(jj))) {
                        ...
                        totals.repetidos++;
                        // Separar la salida si hay cambio de directorio (esto funciona si la vaina est� ordenada)
        ...
        if (compDel.frame.optionPanel.delDirBox.isSelected() && !mustDie) {
            if (dirs != null) {     // delDirBox.isSelected() could have changed, so dirs can be null
                compDel.frame.setTitle(getClass().getSimpleName() + ": borrando directorios vac�os");
	...
	private void setUpFileSets() throws IOException {
		...
		if (archivo1 == null || dir1IsDirty) {
			...
			err.println("El primer directorio ya est� listo:\t" + new Date());
		} else {
			err.println("El primer directorio ya lo ten�a desde antes:\t" + new Date());
		}
		...
		if (dirF1.equals(dirF2)) {
			...
			if (archivo2 == null || dir2IsDirty) {
				...
				err.println("El segundo tambi�n est� listo:\t" + new Date());
			} else {
				err.println("El segundo directorio ya lo ten�a desde antes:\t" + new Date());
			}
		}
	}
	...
	private void generateDigests() {
		...
		err.println("ESTIMAR LA DURACI�N DE LOS DIGESTS ");
        final int cron = Chrono.getChrono();
        ...
        Chrono.mark(cron);
        err.println("DURACI�N DE LOS DIGESTS: "
	...
	private void tryToDeleteFile(final int ii, final int jj) {
		...
		if (compDel.frame.optionPanel.deleteBox.isSelected() && archivo1.get(ii).delete()) {
			out.println(String.valueOf(ii + 1) + "\tBorrado par Co�o -> "
    ...
    /**
     * Si se llaman igual, tienen el mismo tama�o, etc. hacer la comparacion y el borrado sin el digest.
