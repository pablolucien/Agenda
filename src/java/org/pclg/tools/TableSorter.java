 ...
 * and itself implements TableModel. org.pclg.tools.TableSorter does not store or copy
 ...
 * array. That way the org.pclg.tools.TableSorter appears to hold another copy of the table
...
public class TableSorter extends TableMap {
    private static final long serialVersionUID = 827368480293223958L;
    private int[]             indexes;
    ...
    int compareRowsByColumn(final int row1, final int row2, final int column) {
    ...
    int compare(final int row1, final int row2) {
        compares++;
        for (int level = 0; level < sortingColumns.size(); level++) {
            final Integer column = (Integer)sortingColumns.get(level);
	...
	void shuttlesort(final int[] from, final int[] to, final int low,
		final int high) {
        if (high - low < 2) {
            return;
        }
