...
public class ReadAfterWrite {
    ...
    private static class WriteA extends Thread {
	@Override
	public void run() {
	    yield();
	    final int [] mem = BB;
    ...
    private static class WriteB extends Thread {
	@Override
	public void run() {
	    final int [] mem = AA;
	    for(int i = 0; i < n; i++) {
    ...
    private static void dump(final String name, final String name2,
		final int[] mem, final int i) {
	int j = i;
	int count = 0;
	while (j > 1 && count < 4) {
	    if (mem[j-1] != mem[j]) {
	...
	final Thread t1 = new WriteA();
	final Thread t2 = new WriteB();
	t1.start();
	t2.start();
	t1.join();
	t2.join();
