			...
			if (checkInvalidChars) {
				for (final char invalidChar : invalidChars) {
					if (name.indexOf(invalidChar) >= 0) {
						System.out.println("Caracteres inválidos " + file.getAbsolutePath());
						break;
					}
				}
//				if(name.matches(invalidChars)) {
//					System.out.println("Caracteres inválidos " + files[i].getAbsolutePath());
//					continue;
//				}
			}
			...
			if (checkEmptyDirs) {
				if (file.isDirectory()) {
					if (file.list().length == 0) {
						System.out.println("Directorio vacío " + file.getAbsolutePath());
					}
					continue;
				}
			}
			...
			if (checkEmptyFiles) {
				if (file.length() == 0) {
					System.out.println("Archivo vacío " + file.getAbsolutePath());
					continue;
				}
			}
